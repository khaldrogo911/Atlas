...
This repository is at **ATLAS-TASK-0001: Repository Bootstrap & Engineering
Foundation**.
...
configuration and CI. **No trading logic exists yet.** Every package below is
an importable unit with a documented responsibility and no implementation, by
design. `atlas.config` is the single exception — configuration *is* foundation,
so it is fully implemented and tested.
